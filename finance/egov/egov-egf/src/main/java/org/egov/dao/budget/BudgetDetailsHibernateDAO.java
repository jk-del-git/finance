...
public class BudgetDetailsHibernateDAO implements BudgetDetailsDAO {
	...
	/**
	 * This API is to check whether the planning budget is available or not.For the
	 * amount passed if there is sufficient budget available API will return TRUE.
	 * Else it will return FALSE. At any point the budgetavailable will show the
	 * right picture of how much more we can plan for.
	 * <p>
	 * Assumptions- 1) on load of the budget there will be budgetavailable amount
	 * loaded using the multiplier factor. 2) on load of supplementary budget budget
	 * available is recalculated and updated.
	 * <p>
	 * For the sake of audit we should be updating the budgetusage object with the
	 * reference object and the moduleid and amount for any budget consumed. This
	 * will be used for reporting as to which object consumed how much and when.
	 *
	 * @param financialyearid This is the id from the financial year object
	 * @param moduleid        This is the id of the module, say for payroll 7 and
	 *                        for stores 8 and for PTIS it is 2
	 * @param referencenumber This is the module object reference number, say
	 *                        purchase order number or estimate number
	 ...
	 * @param amount          This is the amount of which budget needs to be
	 *                        allocated
	 * @return boolean
	 * @throws Exception
	 */
	...
	/**
	 * Does the same as the above API. Except this API returns BudgetUsage object if
	 * the available amount is >=0. If there is insufficient available amount then
	 * null is returned. Note that the first parameter is a String, unlike the
	 * previous API.
	 * 
	...
	/**
	 * This API will be called for releasing the budget that was already allocated
	 * to some estimate or purchase order. On calling this the amount that was
	 * allocated to some entity will get released. Budget available will be
	 * increased.
	 * <p>
	 * On modify of any entity they should first call the release budget and then
	 * call the consume budget. The budget usage table needs to be updated anytime
	 * this is invoked with the date and reference object number.
	 * 
	 * @param detailsMap is the map containig following fields financialyearid This
	 *                   is the id from the financial year object moduleid This is
	 *                   the id of the module, say for payroll 7 and for stores 8
	 *                   and for PTIS it is 2 referencenumber This is the module
	 *                   object reference number, say purchase order number or
	 *                   estimate number departmentid This is the id of the
	 *                   department object functionid This is the id of the function
	 *                   object functionaryid This is the id of the functionary
	 *                   object schemeid This is the id of the scheme object
	 *                   subschemeid This is the id of the sub scheme object fieldid
	 *                   This is the id of the boundary object budgetheadid This is
	 *                   the id of budgegroup object amount This is the amount of
	 *                   which budget needs to be allocated
	 * @throws Exception
	 */
	...
	private BigDecimal getDetails(final Map<String, Object> detailsMap) {
		...
		try {
           ...
           * LOGGER.error("Exception in consumeEncumbranceBudget API()=" +
	...
	private BigDecimal getDetails(final Long financialyearid, final Integer moduleid, final String referencenumber,
		...
		try {
           ...
           * ValidationException(EMPTY_STRING, e.getMessage()); }
           */
	}
	...
	private BudgetUsage getBudgetUsageDetails(final Long financialyearid, final Integer moduleid,
		...
		try {
		    ...
		    LOGGER.error("Exception in consumeEncumbranceBudget API()=" + v.getMessage());
	...
	private StringBuilder prepareAgregateQuery(final Integer departmentid, final Long functionid,
		...
		if (departmentid != null && departmentid != 0) {
			query.append(" and bd.executingDepartment=:departmentid");
	...
	private String prepareQueryToGetBudgetDetails(final Integer departmentid, final Long functionid,
		...
		if (departmentid != null) {
			query.append(" and bd.executingDepartment=:departmentid");
			params.put("departmentid", departmentid.longValue());
		}
		if (functionid != null) {
			query.append(" and bd.function=:functionid");
			params.put("functionid", functionid);
	...
	public BigDecimal getPlanningBudgetAvailable(final Long financialyearid, final String departmentcode,
		...
		try {
			...
			final StringBuilder finalquery = new StringBuilder(
					"select sum (budgetAvailable) from BudgetDetail bd where bd.budget.isbere=:type")
							.append(" and bd.budget.financialYear.id=:financialyearid")
           ...
           * catch (final Exception e) {
           * LOGGER.error("Exception in getPlanningBudgetAvailable API()=" +
           * e.getMessage()); throw new ValidationException(EMPTY_STRING,
           * e.getMessage()); }
           */
	}
	...
	public List<BudgetDetail> getBudgetAvailableDetail(final Long financialyearid, final Integer departmentid,
		...
		try {
			...
			final Map<String, Object> params = new HashMap<>();
			String query = prepareQueryToGetBudgetDetails(departmentid, functionid, functionaryid, schemeid,
					subschemeid, boundaryid, fundid, params);

			if (budgetheadid != null && !budgetheadid.isEmpty())
				query = query.concat(" and bd.budgetGroup.id in (:budgetheadid) ");
			final String finalquery = new StringBuilder("select bd from BudgetDetail bd where bd.budget.isbere=:type ")
					.append("and bd.budget.financialYear.id=:financialyearid").append(query).toString();
	 ...
	 * return the transaction amount consumed for the current year based on asOnDate
	 * or previous financial year and other given parameters from Generalledger.
	 * Transaction amount calculation for Income and Liabilities -> sum(creditamt) -
	 * sum (debitamt) Transaction amount calculation for Expense and Assets ->
	 * sum(debitamt) - sum (creditamt)
	 * 
	 ...
	 * @param budgetHead    (mandatory) - budget head object, which having the major
	 *                      code/ minor code/ detailcode/ range of minor/detail
	 *                      codes (based on appconfig values)
	 ...
	 * subschemeid,boundaryid,budgetheadid,financialyearid it'll get the budgeted
	 * amount based on the parameters.
	 * 
	 * @param paramMap
	 ...
	 * This parameter HashMap contains deptid,functionid, functionaryid,schemeid,
	 * subschemeid,boundaryid,budgetheadid,financialyearid it'll get the budgeted
	 * amount based on the parameters. Only financial year parameter will be
	 * considered mandatory here.
	 * 
	 * @param paramMap
	 * @return budgeted amount @
	 */
	...
	public Map<String, BigDecimal> getAggregateBudgetedAmtForYear(final Map<String, Object> paramMap) {
		...
		try {
			...
			String finalquery;
			if (budgetService.hasApprovedReForYear(financialyearid))
				finalquery = new StringBuilder(" from BudgetDetail bd where bd.budget.isbere='RE' ").append(query)
						.append(" order by bd.executingDepartment ").toString();
			else
				finalquery = new StringBuilder(" from BudgetDetail bd where bd.budget.isbere='BE' ").append(query)
						.append(" order by bd.executingDepartment ").toString();
			...
			throw new ValidationException(EMPTY_STRING,
                                "Exception in getAggregateBudgetedAmtForYear==" + v.getMessage());
        } /*
           * catch (final Exception e) {
           * LOGGER.error("Exp in getAggregateBudgetedAmtForYear ...==" +
           * e.getMessage()); throw new ValidationException(EMPTY_STRING,
	 ...
	 *                 functionaryid (optional) 6. schemeid (optional) 7.
	 *                 subschemeid (optional) 8. boundaryid (optional) 9. glcode
	 *                 (mandatory) - based on the glcode, we can get the
	 *                 budgetheadid 10. asondate (manadtory) - to get the actuals,
	 *                 we need asondate 11. mis.budgetcheckreq-Boolean- (optional)
	 *                 to skip budget check if set to false.Default is true Budget
	 *                 checking will be enabled or disabled by these levels and in
	 *                 the order a. Application - uses set up "Budget Control Type"
	 *                 b. Voucherlevel - uses budgetcheckreq column of vouchermis
	 *                 table for perticular voucher c. Debit or Credit level - uses
	 *                 Budgetgroup.budgetting type for debit side only ,credit side
	 *                 or both d. Glcode level - Uses chartofaccounts.budgetcheckreq
	...
	/**
	 * To check budget available for the glcode with other parameters. if txnamt is
	 * less than the budget available, it would return true, otherwise false.
	 * 
	 * @param paramMap
	 * @return @
	 */
	...
	/**
	 * To get the Budgetgroup for the glcode at detailcode level or minorcode level
	 * or major code level.
	 * 
	 * @param coa
	 * @param paramMap
	...
	public List<BudgetGroup> getBudgetHeadByGlcode(final CChartOfAccounts coa) {
		try {
			...
			StringBuilder query = new StringBuilder(" from BudgetGroup bg where bg.minCode.glcode<=?")
					.append(" and bg.maxCode.glcode>=?")
			...
			if (bgList.isEmpty()) {
				query = new StringBuilder(" from BudgetGroup bg where bg.minCode.glcode<=?")
						.append(" and bg.maxCode.glcode>=?")
						.append(" and bg in (select budgetGroup from BudgetDetail) and bg.isActive=true");
				if (LOGGER.isDebugEnabled())
				...
				if (bgList.isEmpty()) {
					query = new StringBuilder(" from BudgetGroup bg where bg.majorCode.glcode=?")
							.append(" and bg in (select budgetGroup from BudgetDetail) and bg.isActive=true ");
					// persistenceService.setType(BudgetGroup.class);
					bgList = persistenceService.findAllBy(query.toString(),
           ...
           * catch (final Exception e) {
           * LOGGER.error("Exception in getBudgetHeadByGlcode API=" +
           * e.getMessage()); throw new ValidationException(EMPTY_STRING,
           * e.getMessage()); }
           */
	}
	...
	/**
	 * To get the Budgetgroup for the glcode at detailcode level for a list of COA
	 * codes
	 * 
	 * @param coa
	 * @param paramMap
	...
	public List<BudgetGroup> getBudgetHeadForGlcodeList(final List<CChartOfAccounts> coa) {
		try {
			List<String> coaList = new ArrayList<>();
			if (coa.isEmpty()) {
				if (LOGGER.isDebugEnabled())
				...
				for (Integer i = 0; i < coa.size(); i++)
					coaList.add(coa.get(i).getGlcode());
			}
			final StringBuilder query = new StringBuilder(" from BudgetGroup bg where bg.minCode.glcode in (?)")
			...
			// persistenceService.setType(BudgetGroup.class);
			final List bgList = persistenceService.findAllBy(query.toString(), coaList);
			return bgList;
           ...
           * catch (final Exception e) {
           * LOGGER.error("Exception in getBudgetHeadForGlcodeList API=" +
           * e.getMessage()); throw new ValidationException(EMPTY_STRING,
           * e.getMessage()); }
           */
	}
	 ...
	 *                 (mandatory) - based on the glcode, we can get the
	 *                 budgetheadid 10. asondate (manadtory) - to get the actuals,
	 *                 we need asondate 11.mis.budgetcheckreq-Boolean-(optional) set
	 *                 to false if budget check not to be done for this bill default
	 *                 is True.
	 * @return @
	 */
	@Override
...
//						LOGGER.debug("In exception thow of budget check at line 2200");
	...
	/**
	 * This parameter HashMap contains deptid,functionid, functionaryid,schemeid,
	 * subschemeid,boundaryid,budgetheadid,financialyearid,typeBeRe it'll get the
	...
	public BigDecimal getBudgetedAmtForYearRegardingBEorRE(final Map<String, Object> paramMap, final String typeBeRe) {
		...
		try {
			...
			if (budgetheadid == null)
				throw new ValidationException(EMPTY_STRING, "Budget head id is null or empty");
			query.append(" and bd.budgetGroup=:budgetheadid");
			params.put("budgetheadid", budgetheadid);

			if (financialyearid == null)
				throw new ValidationException(EMPTY_STRING, "Financial Year id is null");
			query.append(" and bd.budget.financialYear=:financialyearid");

			final String finalquery = new StringBuilder(" from BudgetDetail bd where bd.budget.isbere= :typeBeRe ")
					.append(query).toString();

			params.put("typeBeRe", typeBeRe);
			if (LOGGER.isDebugEnabled())
				LOGGER.debug("finalquery  =" + finalquery);
			final Query qry = persistenceService.getSession().createQuery(finalquery);
			params.entrySet().forEach(entry -> qry.setParameter(entry.getKey(), entry.getValue()));
			final List<BudgetDetail> bdList = qry.list();
	...
	/**
	 * @description - returns the Sanctioned Planning Budget amount based on various
	 *              parameters. For an year if there is an approved RE, then the RE
	 *              sanctioned planning amount will be returned else the sanctioned
	 *              BE amount will be returned. This will take care of the
	 *              re-appropriations also.
	 ...
	 *                      required for the query the queryParamMap contain values
	 *                      :- the mis attribute values passed in the query map will
	 *                      be validated with the appconfig value of
	 *                      key=budgetaryCheck_groupby_values financialyearid -
	 *                      optional ExecutionDepartmentId - mandatory
	 *                      -if:department present in the app config value -
	 *                      optional -else fundId - mandatory -if:fund present in
	 *                      the app config value - optional -else schemeId -
	 *                      mandatory -if:Scheme present in the app config value -
	 *                      optional -else functionId - mandatory -if:function
	 *                      present in the app config value - optional -else
	 *                      subschemeId - mandatory -if:Subscheme present in the app
	 *                      config value - optional -else functionaryId - mandatory
	 *                      -if:functionary present in the app config value -
	 *                      optional -else boundaryId - mandatory -if:boundary
	 *                      present in the app config value - optional -else
	 *                      moduleId - optional financialYearId -optional
	 *                      budgetgroupId -optional fromDate -optional toDate
	 *                      -optional Order By - optional if passed then only
	 *                      Budgetary appropriation number and reference number is
	...
	public List<BudgetUsage> getListBudgetUsage(final Map<String, Object> queryParamMap) {
		...
		if (!isNull(grpByVls.get("department")))
			if (isNull(queryParamMap.get("ExecutionDepartmentId")))
				...
				params.put("executingDepartmentId",
		...
		if (!isNull(grpByVls.get("boundary")))
			if (isNull(queryParamMap.get("boundaryId")))
				...
				query.append(" and bd.boundary.id=:boundaryId");
				params.put("boundaryId", Integer.valueOf(queryParamMap.get("boundaryId").toString()));
			}
	...
	public BigDecimal getPlannigBudgetBy(final Long fundId, final Integer deptId, Date asOnDate1) {
		...
		amount = (BigDecimal) persistenceService.find(
				new StringBuilder("select sum(approvedAmount) from BudgetDetail bd")
						.append(" where  bd.executingDepartment.id=? and bd.fund.id=?")
						.append(" and bd.budget.financialYear=? and bd.budget.isbere=? ").toString(),
		...
		BigDecimal reappAmount = (BigDecimal) persistenceService.find(
				new StringBuilder("select sum(additionAmount-deductionAmount) from BudgetReAppropriation br")
						.append(" where  br.budgetDetail.executingDepartment.id=? and br.budgetDetail.fund.id=?")
						.append(" and br.budgetDetail.budget.financialYear=? and br.budgetDetail.budget.isbere=?")
						.append(" and br.status.description='Approved' and to_date(br.modifiedDate)<=? ").toString(),
	...
	/**
	 * Returns a list of Functions having entry in budget detail with the given fund
	 * and department .
	 * 
